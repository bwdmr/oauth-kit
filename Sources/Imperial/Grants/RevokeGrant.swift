...
public protocol RevokeGrant: ImperialGrantable { }

extension RevokeGrant {
  public mutating func flow(req: Request, requestBody: ImperialToken) async throws {
    guard
      let _ = requestBody.token
    ...
    components.queryItems = [
      requestBody.tokenItem ]
    
    guard let url = components.url else { throw Abort(.notFound) }
    let urlString = url.absoluteString
    let uri = URI(string: urlString)
    let revoketokenResponse = try await req.client.post(uri, beforeSend: { req in
      req.headers.contentType = .urlEncodedForm
      try req.content.encode(requestBody)
    }).encodeResponse(for: req)
    
    let revoketokenBody = ImperialToken()
    try await handler(req, revoketokenBody)
  }
}
